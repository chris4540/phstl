...
class STLWriter():
    ...
    def add_facet(self, t):
        ...
        # strictly speaking, we don't need to compute normal_vector,
        # as other tools could be used to update the output mesh.
        self.f.write(pack('<3f', *normal_vector(t)))
        for vertex in t:
...
if __name__ == '__main__':
    ...
    ap.add_argument('-x', action='store', default=0.0, type=float,
                    help='Fit output x to extent (mm)')
    ap.add_argument('-y', action='store', default=0.0, type=float,
                    help='Fit output y to extent (mm)')
    ...
    optg_region.add_argument(
        '-w', '--window', action='store', default=None, type=float, nargs=4,
        help='Opposing corner coordinates in geographic CRS')
    optg_region.add_argument(
        '-p', '--pixels', action='store', default=None, type=float, nargs=4,
    ...
    # setup logger and the desire log level
    if args.verbose:
    ...
    w = img.RasterXSize
    h = img.RasterYSize
    log.debug("raster dimensions = (%s, %s)" % (str(w), str(h)))
    ...
    log.debug("geo->pixel transform: %s" % str(git))
    ...
    log.debug("xmin, ymin = %d, %d" % (xmin, ymin))
    log.debug("ww, wh = %d, %d" % (ww, wh))
    ...
    if args.x != 0.0 or args.y != 0.0:
        ...
        # recalculate xy scale based on requested x or y dimension
    ...
    log.debug("data type = %s" % typeName)
    log.debug("type format = %s" % typemap.get(band.DataType))

    # min, max, mean, sd; min used for z clipping
    stats = band.GetStatistics(True, True)
    log.debug("min, max, mean, sd = %s" % str(stats))
    ...
    log.debug("buffer size = %s" % str(pixels.maxlen))
    ...
    log.debug("predicted (max) facet count = %s" % str(facetcount))
    log.debug("predicted (max) STL file size = %s bytes" % str(filesize))
    ...
    def skip(v):
        ...
        # global args
        ret = False
        if v == nd:  # in case of nodata
            ret = True
        if args.minimum and v < args.minimum:
            ret = True
        if args.maximum and v > args.maximum:
            ret = True
        return ret
    ...
    log.debug("actual facet count: %s" % str(mesh.written))
